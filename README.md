...
- **多模型支持** - 支持本地 Ollama 和云端 OpenAI API，可在设置中切换
- **Ollama 集成** - 已接入本地 Ollama API，支持流式响应
- **云端 API** - 支持 OpenAI 兼容的云端模型调用，效果更好
...
- **模型路径** - 自定义模型文件夹路径
- **窗口大小** - 可调整窗口大小（300×500 ~ 800×1200）
- **缩放比例** - 调整模型大小（0.5x - 2.0x）
- **位置偏移** - 调整模型水平和垂直位置
- **气泡颜色** - 自定义气泡和发送按钮颜色（毛玻璃效果，15% 不透明度）
- **女友名字** - 给女友起个名字
- **角色设定** - 自定义角色的性格、说话风格等
- **AI 模型配置** - 选择本地 Ollama 或云端 API，配置模型和 API Key
- **注视鼠标** - 开关鼠标追踪功能
- **记忆管理** - 查看、编辑、删除记忆
...
- **Ollama** - 本地大模型推理引擎（Phase 1-4）
- **better-sqlite3** - SQLite 数据库（记忆存储）
- **nomic-embed-text-v2-moe** - 向量嵌入模型

### AI Provider 架构（Phase 5）

系统支持统一的 AI Provider 接口，可灵活切换不同的模型服务：
...
| OpenAI | ✅ 已支持 | 云端 API，效果更好但需联网 |
| Azure OpenAI | 🟡 规划 | 企业级部署 |
| Claude | 🟡 规划 | Anthropic 模型兼容 |
...
```json
{
  ...
  "dailyFeatures": {
    "enabled": false,
    "morningMessage": "早安~亲爱的，今天也要加油哦！",
    "nightMessage": "晚安~好梦哦，梦里有我~",
    "morningTime": "08:00",
    "nightTime": "22:00",
...
- [x] 创建统一的 AI Provider 接口
- [x] 实现 OpenAI API 兼容调用
- [x] 实现 Ollama API 兼容调用
- [x] 支持 API Key 配置
- [x] 支持自定义 API 地址
- [x] 支持模型选择切换
- [x] 向量嵌入服务统一适配
...
### 2026-04-22 🎉 **Phase 5 完成！云端模型支持上线！**

- ✅ **AI Provider 架构** - 统一的 AI Provider 接口，支持多种模型服务
- ✅ **OpenAI API 支持** - 兼容 OpenAI API 格式的云端模型调用
- ✅ **Ollama 本地支持** - 保持本地 Ollama 推理能力
- ✅ **设置面板集成** - 可视化配置 AI 模型（本地/云端切换）
- ✅ **API Key 管理** - 安全存储云端 API Key
- ✅ **向量嵌入统一** - embeddingService 支持云端模型
- ✅ **模型选择** - 支持自定义聊天模型和嵌入模型

### 2026-04-21 🎉 **窗口大小调整 & 系统托盘功能上线！**

- ✅ **窗口大小可调整** - 用户可在设置中调整窗口大小（300×500 ~ 800×1200）
...
### 2026-04-13 🎉 **Phase 4 完成！记忆管理界面上线！**
